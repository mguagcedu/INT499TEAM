...
echo "Rewriting src/pages/MoviesPage.jsx with a clean version..."
...
export default function MoviesPage({
  ...
  // Pick up ?query= from the URL (from StreamList page)
  useEffect(() => {
  ...
  // Simple carousel that rotates through current results
  useEffect(() => {
    ...
    return (
      ...
      >
        ...
        <div style={{ display: "flex", gap: "8px", marginTop: "4px" }}>
          ...
          >
            Add to Cart{" "}
            {cartCount(movie.id) > 0 ? `(${cartCount(movie.id)})` : ""}
          </button>
        </div>
      </div>
    );
  ...
  return (
    <div style={{ padding: "16px" }}>
      ...
      <p style={{ fontSize: "14px", color: "#9ca3af", marginBottom: "16px" }}>
        This page shows live results from the TMDB catalog only. When you
        search, the carousel updates and rotates through the top matches.
      </p>
      ...
      {featured && (
        ...
        >
          ...
          {/* Smaller centered card so poster is not oversized */}
          <div
            style={{
              maxWidth: "260px",
              margin: "0 auto"
...
echo "MoviesPage.jsx successfully rewritten."
